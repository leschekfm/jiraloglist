...
async function gll (config, dayToCheck = moment().startOf('day').subtract(1, 'd'), range = 1) {
  fetchGitlabData(config, dayToCheck, range)
}

async function fetchGitlabData (config, dayToCheck, range) {
  ...
  gttConfig.set('from', dayToCheck.format())
  gttConfig.set('to', dayToCheck.add(range, 'd').format())
