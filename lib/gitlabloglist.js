...
async function fetchGitlabData(config, dayToCheck) {
    ...
    gttConfig.set('url', 'https://gitlab.com/api/v4/')
    gttConfig.set('token', 'XXX');
    gttConfig.set('project', 'namespace/project');
    gttConfig.set('closed', true);
    gttConfig.set('output', 'markdown')
    ...
    output = new Output[gttConfig.get('output')](gttConfig, report);
    output.make();
    output.toStdOut();
    ...
    function sendSlackmessage(message, config) {
        ...
        debug(message)
        web.chat.postMessage(config.slack.channel, message, (err, res) => {
            if (err) {
                throw new Error(err)
            }
            debug(res)
