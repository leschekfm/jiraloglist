  ...
  return new Promise((resolve, reject) => {
    req.get({
      ...
      qs: {
        ...
        fields: 'summary,status,worklog,timetracking',
        maxResults: 1000
      }
    }, (err, res, body) => {
      if (err) {
...
function generateSlackMessage (tracking, dayToCheck, range) {
  ...
  for (const user in tracking) {
    ...
    for (const issueKey in tracking[user].issues) {
      ...
      const emoji = chooseEmoji(issue)
      fields.push({
        title: `${issueKey} - ${issue.desc}`,
        value: `${issue.timeSpent / 60} mins, Remaining: ${issue.remaining} ${emoji}`,
        short: true
      })
    }
...
function chooseEmoji(issue) {
  switch (issue.status) {
    ...
    default:
    return issue.remaining === '0m' ? ':warning:' : ''
      break;
  }
}
